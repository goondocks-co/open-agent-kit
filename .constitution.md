...
```bash
open-agent-kit/
├── src/open_agent_kit/               # Source code (package root)
...
│   │   ├── feature_service.py
│   │   └── template_service.py
│   ├── agents/            # Agent manifests (included in package)
│   │   ├── claude.yaml
│   │   ├── copilot.yaml
│   │   └── ...
│   ├── features/          # Feature packages (included in package)
│   │   ├── core/         # Core assets (IDE settings)
│   │   │   ├── manifest.yaml
│   │   │   └── ide/
│   │   ├── rules_management/  # Rules/constitution feature
│   │   │   ├── manifest.yaml
│   │   │   ├── constitution.py  # ConstitutionService
│   │   │   ├── commands/
│   │   │   └── templates/
│   │   ├── strategic_planning/  # RFC and planning feature
│   │   │   ├── manifest.yaml
│   │   │   ├── rfc.py      # RFCService
│   │   │   ├── plan/       # PlanService
│   │   │   ├── commands/
│   │   │   └── templates/
│   │   └── codebase_intelligence/  # Codebase analysis feature
│   │       ├── manifest.yaml
│   │       ├── service.py
│   │       └── hooks/
...
**Note:** Feature directories use underscores (Python package convention) but canonical
feature names use hyphens (e.g., `strategic-planning` → `strategic_planning/`).
Feature-specific services are co-located with their feature code (vertical slice).
...
1. Create `src/open_agent_kit/features/<name_underscored>/manifest.yaml`
2. Add command templates in `src/open_agent_kit/features/<name_underscored>/commands/`
3. Add document templates in `src/open_agent_kit/features/<name_underscored>/templates/`
4. Add feature-specific services in the same directory (vertical slice pattern)
5. Update constants (`SUPPORTED_FEATURES`, `FEATURE_CONFIG`)
6. Write tests
7. Update documentation

**Note:** Directory names use underscores (`strategic_planning/`) but manifest names
use hyphens (`name: strategic-planning`).
...
1. **Command Templates** - For AI agent commands (in `src/open_agent_kit/features/<feature>/commands/`)
   - `oak.rfc-create.md` - RFC creation assistant
   - `oak.rfc-validate.md` - RFC validation assistant

2. **Document Templates** - For generated documents (in `src/open_agent_kit/features/<feature>/templates/`)
...
- v3.2.0 (2025-01-19) - Vertical slice architecture: moved agents/ and features/ inside src/open_agent_kit/ for proper package inclusion; co-located feature-specific services with their features; updated Sections II.1, VI.5, VII.1
