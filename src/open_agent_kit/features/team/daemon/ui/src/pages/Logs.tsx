...
export default function Logs() {
    ...
    /** Filter log lines by selected tags (OR logic - matches any selected tag) */
    const filterLogsByTags = (logLines: string[]): string[] => {
        if (selectedTags.size === 0) return logLines;
                ...
                <CardContent className="flex-1 p-0 overflow-hidden bg-black text-green-400 font-mono text-xs rounded-b-lg">
                    {isLoading ? (
                        ...
                        <div className="overflow-auto h-full p-4 whitespace-pre-wrap">
                            {(() => {
                                const filtered = filterLogsByTags(data?.lines ?? []);
